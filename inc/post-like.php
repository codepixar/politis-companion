...
add_action( 'wp_enqueue_scripts', 'politis_sl_enqueue_scripts' );
function politis_sl_enqueue_scripts() {

	wp_enqueue_script( 'politis-post-likes', plugins_url( '../js/post-likes.js', __FILE__ ), array( 'jquery' ), '0.5', false );

	wp_localize_script( 'politis-post-likes', 'simpleLikes', array(
		'ajaxurl' => admin_url( 'admin-ajax.php' ),
		'unlike'  => esc_html__( 'Unlike', 'politis-core' )
	) );
}
 ...
 * @since    0.5
 */
add_action( 'wp_ajax_nopriv_politis_process_simple_like', 'politis_process_simple_like' );
add_action( 'wp_ajax_politis_process_simple_like', 'politis_process_simple_like' );
function politis_process_simple_like() {
	// Politis
	$nonce = isset( $_REQUEST['nonce'] ) ? sanitize_text_field( $_REQUEST['nonce'] ) : 0;
	if ( !wp_verify_nonce( $nonce, 'simple-likes-nonce' ) ) {
		exit( esc_html__( 'Not permitted', 'politis-core' ) );
	}
...
function get_simple_likes_button( $post_id, $is_comment = NULL ) {
	...
	$output = '';
	$nonce = wp_create_nonce( 'simple-likes-nonce' ); // Politis
	if ( $is_comment == 1 ) {
	...
	if ( already_liked( $post_id, $is_comment ) ) {
		...
		$title = esc_html__( 'Unlike', 'politis-core' );
		$icon = $icon_full;
	} else {
		$class = '';
		$title = esc_html__( 'Like', 'politis-core' );
		$icon = $icon_empty;
	}
...
function get_like_count( $like_count ) {
	$like_text = esc_html__( '', 'politis-core' );
...
// User Profile List
add_action( 'show_user_profile', 'politis_show_user_likes' );
add_action( 'edit_user_profile', 'politis_show_user_likes' );
function politis_show_user_likes( $user ) { ?>        
	<table class="form-table">
		<tr>
			<th><label for="user_likes"><?php _e( 'You Like:', 'politis-core' ); ?></label></th>
			<td>
			<?php
			...
			<?php else : ?>
			<p><?php esc_html__( 'You do not like anything yet.', 'politis-core' ); ?></p>
			<?php 
			endif; 
			wp_reset_postdata(); 
			?>
